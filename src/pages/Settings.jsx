      ...
      <div className="mt-6 space-y-3">
        ...
        {syncReady && (
          <div className="bg-white rounded-2xl p-4 shadow-[0_4px_12px_rgba(199,91,60,0.08)]">
            ...
            <p className="text-xs text-charcoal-light mt-1">
              {lang === 'es'
                ? 'Usa el mismo código en ambos dispositivos para compartir plan, compra y gastos.'
                : 'Use the same code on both devices to share plan, shopping and spending.'}
            </p>
            <div className="flex gap-2 mt-3">
              ...
              />
              <button onClick={generateCode} className="px-3 py-2 border border-cream-dark rounded-xl text-xs font-heading text-charcoal-light">
                🎲
              </button>
              <button onClick={handleSaveCode} className="bg-terra text-white px-4 py-2 rounded-xl font-heading text-xs font-semibold">
                {syncSaved ? '✓' : (lang === 'es' ? 'Guardar' : 'Save')}
              </button>
            </div>
            {getFamilyCode() && (
              <p className="text-[10px] text-forest mt-2">
                ✓ {lang === 'es' ? `Sincronizado: ${getFamilyCode()}` : `Synced: ${getFamilyCode()}`}
              </p>
            )}
          </div>
        )}
        ...
        {!syncReady && (
          <div className="bg-white rounded-2xl p-4 shadow-[0_4px_12px_rgba(199,91,60,0.08)]">
            ...
            <p className="text-xs text-charcoal-light mt-1">
              {lang === 'es'
                ? 'No configurada. Los datos se guardan solo en este dispositivo.'
        ...
        <div className="bg-white rounded-2xl p-4 shadow-[0_4px_12px_rgba(199,91,60,0.08)] flex items-center justify-between">
          ...
          <button onClick={toggleLanguage} className="bg-terra text-white px-4 py-2 rounded-xl font-heading text-xs font-semibold active:scale-95 transition-transform">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-[0_4px_12px_rgba(199,91,60,0.08)] flex items-center justify-between">
          ...
          <button onClick={toggleSibo} className={`w-14 h-8 rounded-full relative transition-colors ${siboMode ? 'bg-forest' : 'bg-cream-dark'}`}>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-[0_4px_12px_rgba(199,91,60,0.08)] flex items-center justify-between">
          ...
          <button onClick={toggleDark} className={`w-14 h-8 rounded-full relative transition-colors ${darkMode ? 'bg-forest' : 'bg-cream-dark'}`}>
